...
class Solution:
    ...
    def ObtainComponents(self, canNewComponents):
        ...
        for component in range(0, canNewComponents):
            component = Component(
                siteId, self.Problem.totalWeeks, vigilantsByPeriodInSite)
            self.getSchedule(component, shifts,
                             copy.deepcopy(necesaryVigilantes))
        ...
        for shift in shifts:
            ...
            for iteration in range(0, component.necesaryvigilantsByPeriod[shift[0]]):
                ...
                self.AssigmentVigilants(
                    objViglant, component.siteId, shift, component)
    ...
    def tweakVigilants(self, solucion):
        if self.is_empty(solucion.vigilantsForPlaces):
            ...
            vigilantOne = self.getAleatory(
                0, len(solucion.vigilantsForPlaces[listSite[0]]), 1)
            vigilantTwo = self.getAleatory(
                0, len(solucion.vigilantsForPlaces[listSite[1]]), 1)
            self.toExchageVigilants(
                listSite[0], vigilantOne[0], listSite[1], vigilantTwo[0], solucion)
        return solucion
